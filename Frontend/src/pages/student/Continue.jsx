...
// import { BadgeInfo, CheckCircle, CheckCircle2, CirclePlay, Download, DownloadCloud } from "lucide-react";
...
//       try {
//         const response = await fetch(`${BasedUrl}/api/v1/course/getuser`, {
...
//     toast.success("notes is downloaded.....");
//   };
...
//                       <Badge
//                         variant={"outline"}
//                         className="bg-green-200 text-green-600 flex-col align-center justify-end p-2"
//                       >
...
const Continue = () => {
  ...
  useEffect(() => {
    const fetchLectureData = async () => {
      try {
        ...
        if (!response.ok) throw new Error("Failed to fetch lecture info");
 ...
 const link = document.createElement("a");
  link.href = "/public/resume3.pdf"; // Replace with the actual path to your notes PDF
  link.setAttribute("download", `${courseTitle}.pdf`);
  ...
  const handleDownloadVideo = async (videoUrl, filename = "lecture.mp4") => {
    try {
    ...
    <div className="max-w-7xl mx-auto p-4 my-10">
      ...
      <div className="flex flex-col md:flex-row gap-2">
        <div className="flex-1 rounded-lg shadow-lg p-4">
          <Card className="dark:border-gray-700">
            <CardContent className="p-4 flex flex-col">
              ...
              {/* ✅ Video Download Button Below Player */}
        ...
        <div className="flex flex-col w-full md:w-2/5 border-t md:border-t-0 md:border-l border-gray-700 md:pl-4 pt-4 md:pt-0">
          <h2 className="font-semibold text-xl mb-4">Lectures....</h2>
          <div className="flex-1 overflow-y-auto">
            {courseDetails?.lectures.map((lecture) => (
              <Card
                key={lecture._id}
                className={`mb-3 hover:cursor-pointer transition transform dark:border-gray-700 ${
                  lecture._id === currentLecture?._id
                    ? "bg-gray-200 dark:dark:bg-gray-800"
                    : ""
                } `}
              ...
              >
                <CardContent className="flex items-center justify-between p-4">
                  ...
                  {isLectureCompleted(lecture._id) && (<div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDownloadVideo(lecture.videoUrl, `${lecture.lectureTitle}.mp4`);
                      }}
                    >
                      <Badge
                        variant={"outline"}
                      ...
                      >
                        <DownloadCloud size={16} className="text-green-500" />
                      </Badge>
                    </Button>
                  </div>)}
                </CardContent>
